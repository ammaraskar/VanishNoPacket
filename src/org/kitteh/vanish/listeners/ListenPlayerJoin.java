...
public class ListenPlayerJoin implements Listener {
    ...
    public void onPlayerJoinLate(PlayerJoinEvent event) {
        if (VanishPerms.joinVanished(event.getPlayer())) {
            ...
            event.getPlayer().sendMessage(ChatColor.DARK_AQUA + Messages.getString("ListenPlayerJoin.youJoinedVanished") + add);
            this.plugin.messageStatusUpdate(ChatColor.DARK_AQUA + event.getPlayer().getName() + " " + Messages.getString("ListenPlayerJoin.hasJoinedVanished"));
        }
        ...
        if (VanishPerms.canReceiveAdminAlerts(event.getPlayer()) && this.plugin.versionDifference()) {
            event.getPlayer().sendMessage(ChatColor.AQUA + Messages.getString("ListenPlayerJoin.thisIsVersion") + " " + ChatColor.DARK_AQUA + this.plugin.getCurrentVersion() + ChatColor.AQUA + ", " + Messages.getString("ListenPlayerJoin.latestIs") + " " + ChatColor.DARK_AQUA + this.plugin.getLatestKnownVersion());
            event.getPlayer().sendMessage(ChatColor.AQUA + Messages.getString("ListenPlayerJoin.check") + " " + ChatColor.DARK_AQUA + "http://dev.bukkit.org/server-mods/vanish/");
        }
